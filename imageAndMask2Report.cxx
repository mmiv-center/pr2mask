...
int main(int argc, char *argv[]) {
  ...
  versionString = std::string("0.0.4.") + boost::replace_all_copy(std::string(__DATE__), " ", ".");
  ...
  command.SetOption("TextTopRight", "k", false, "Free text in top-right corner specifying the dependent source data (SeriesNumber, SequenceName). For this field newlines coded as '\n' are allowed.");
  ...
  std::string textTopRight = "";
  if (command.GetOptionWasSet("TextTopRight")) {
    textTopRight = command.GetValueAsString("TextTopRight", "value");
    //textTopRight.erase(remove( textTopRight.begin(), textTopRight.end(), '\"' ), textTopRight.end());
    if (verbose) {
...
*/
        ...
        report->pr2maskVersionString = versionString;
        report->ContainerVersionString = std::string("");
        if (const char *env_p = std::getenv("CONTAINER_VERSION")) { // part of the Dockerfile for building this application
          report->ContainerVersionString = std::string(env_p);
        }
