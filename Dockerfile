...
RUN apt-get update -qq && apt-get install -yq build-essential \
    ...
    libboost-system1.83-dev libboost-date-time1.83-dev libboost-regex1.83-dev libtbb-dev \
    libboost-program-options1.83-dev libfreetype-dev libxml2-dev zlib1g-dev \
    libzip-dev zipcmp zipmerge ziptool

# install itk
RUN cd /tmp/ \
    && wget https://github.com/InsightSoftwareConsortium/ITK/releases/download/v5.4.4/InsightToolkit-5.4.4.tar.gz \
    && cd /opt/ && tar xzvf /tmp/InsightToolkit-5.4.4.tar.gz && cd /opt/InsightToolkit-5.4.4 \
    ...
    && rm /tmp/InsightToolkit-5.4.4.tar.gz
